...
describe('VIRTUALS', () => {
    ...
    it('Virtuals work on queried data: find', async () => {
        ...
        expect(models[0].virtualProp).toBe(
            'This is a virtual prop from model 1.',
        );
        expect(models[1].virtualProp).toBe(
            'This is a virtual prop from model 2.',
        );

        const model = await ModelType.find(model1);

        expect(model[0].virtualProp).toBe(
            'This is a virtual prop from model 1.',
        );
    });
